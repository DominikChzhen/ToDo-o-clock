...
function saveModalTask() {
    ...
    const taskDuration = document.getElementById('TaskTime').value; 
...
function loadTasks() {
    ...
    taskList.innerHTML = ''; // Leere Liste vor dem Laden
    ...
    tasks.forEach((task, index) => {
        ...
        let formattedDueDate = 'Kein Datum angegeben';
        if (task.dueDate) {
        ...
        // Kombinierte Detailanzeige
        const detailsText = `Zeitaufwand: ${task.duration} Min. | Priorität: ${getEisenhowerPriorityLabel(task)} | Fällig: ${formattedDueDate}`;
        ...
        // Baue das gesamte taskItem mit den neuen Elementen zusammen
        taskItem.appendChild(iconElement);
        taskItem.appendChild(textElement);
        taskItem.appendChild(detailsElement);  // Füge die Details hinzu
        taskItem.appendChild(progressBar);
        taskItem.appendChild(completeButton);
        taskItem.appendChild(deleteButton);
        taskItem.appendChild(editButton);
        
        taskList.appendChild(taskItem); // Füge das Element zur Liste hinzu
    });
}
...
function openTimerPopup() {
    const timerPopup = window.open('Pomodoro.html', 'Pomodoro Timer', 'width=400,height=300');
